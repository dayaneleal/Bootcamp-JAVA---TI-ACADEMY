...
--***************************************************************************************
SELECT ioc.idOrdemCompra, ioc.quantidade, ioc.idMaterial, ioc.valor, m.nome, f.nome 
FROM item_ordem_compra as ioc
INNER JOIN material as m 
ON m.id = ioc.idMaterial
INNER JOIN ordem_compra as oc
ON oc.id = ioc.idOrdemCompra
INNER JOIN fornecedor as f
ON f.id = oc.idFornecedor WHERE
quantidade > 100 AND valor < 50
-- *****************************************************************************************
...
-- *****************************************************************************************
SELECT ioc.idOrdemCompra, ioc.quantidade, ioc.idMaterial, 
ioc.valor, m.nome, f.nome as fornecedor, ioc.quantidade*ioc.valor as Subtotal 
FROM item_ordem_compra as ioc
INNER JOIN material as m 
ON m.id = ioc.idMaterial
INNER JOIN ordem_compra as oc
...
SELECT oc.id,f.nome as fornecedor, sum(valor * quantidade) as total 
